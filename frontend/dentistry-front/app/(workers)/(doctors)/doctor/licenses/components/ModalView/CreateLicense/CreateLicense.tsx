...
import { CreateLicense, CreateLicensePayload } from "@/lib/redux/modules/Licenses/actions/CreateLicense/CreateLicense";
...
export default function CreateLicenseModal({
  ...
  const onSubmit = async (values: CreateLicensePayload, { setSubmitting }: any) => {
    try {
      await dispatch(
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-gray-200 p-6 rounded w-96 text-gray-600">
            ...
            {errors.number_license && touched.number_license && (
              <div className="text-red-500 text-sm">{errors.number_license}</div>
            )}
            ...
            {errors.expiration_date && touched.expiration_date && (
              <div className="text-red-500 text-sm">{errors.expiration_date}</div>
            )}
