    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="bg-linear-to-r from-[#874FD1] to-[#7562A5] p-4 rounded shadow-lg w-[90%] max-w-md border border-gray-300 text-base">
        <h3 className="text-base font-bold mb-2">
          {t("admins.calendar.appointment.title")}
        </h3>
        <p className="mb-2">
          <strong>{t("admins.calendar.appointment.patient")}:</strong>{" "}
          {appointment.client?.surname} {appointment.client?.name}{" "}
          {appointment.client?.middle_name}
        </p>
        <p className="mb-2">
          <strong>{t("admins.calendar.appointment.doctor")}:</strong>{" "}
          {appointment.dentist?.surname} {appointment.dentist?.name}{" "}
          {appointment.dentist?.middle_name}
        </p>
        <p className="mb-2">
          <strong>{t("admins.calendar.appointment.date")}:</strong>
          {new Date(appointment.appointment_date).toLocaleString()}
        </p>
        <p className="mb-2">
          <strong>{t("admins.calendar.appointment.status")}:</strong>{" "}
          {appointment.status}
        </p>
        <p className="mb-2">
          <strong>{t("admins.calendar.appointment.notes")}:</strong>{" "}
          {appointment.notes}
        </p>
        ...
        >
          {t("admins.calendar.appointment.close")}
        </button>
      </div>
    </div>
  );
