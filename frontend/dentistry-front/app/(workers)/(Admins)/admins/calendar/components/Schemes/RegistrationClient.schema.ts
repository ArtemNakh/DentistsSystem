...
import { BloodSign } from "@/lib/redux/modules/Clients/clients.interface";
...
export const RegisterClientSchema = Yup.object().shape({
  ...
  blood_resus: Yup.mixed<BloodSign.minus | BloodSign.plus>()
    .oneOf([BloodSign.minus, BloodSign.plus], () =>
  ...
  blood_group: Yup.number()
    .oneOf([1, 2, 3, 4], () =>
      i18n.t("error.registration_client.bloodGroup.invalid"),
    )
  ...
  password: Yup.string()
    ...
    .matches(/[A-Z]/, () =>
      i18n.t("error.registration_client.password.uppercase"),
    )
    .matches(/[a-z]/, () =>
      i18n.t("error.registration_client.password.lowercase"),
    )
  ...
  passwordRepeat: Yup.string()
    ...
    .required(() =>
      i18n.t("error.registration_client.passwordRepeat.required"),
    ),
});
